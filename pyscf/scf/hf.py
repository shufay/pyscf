...
SWITCH_SIZE = getattr(__config__, 'dft_numint_SWITCH_SIZE', 3000)
...
def eval_rho(ao, dm_kpts, weights=1., cell='unit', verbose=False):
    """
    Calculate the electron density for LDA functional.

    Args:
      ao : :class:`numpy.ndarray`
          An array of size (ngrid, nao) that stores the AOs at each grid point.

      dm_kpts : :class:`numpy.ndarray`
          Array of size (2, nkpoints, nao, nao) that stores the 1-electron density matrices
          in plane wave basis.

    Returns:
      List of arrays of size `ngrid` that stores the electron density for spin up and down
      at each grid point.

    Examples:
    >>> kvecs = moire_ham.basis @ moire_ham.prim_reciprocal_vecs.T
    >>> grid = numpy.random.rand((100, 2))
    >>> ao = eval_ao(kvecs, grid, moire_ham.area)
    >>> rho = eval_rho(ao, dm_kpts)
    """
    ngrid, nao = ao.shape

    # Sum over kpoints to get the total density matrix.
    nkpts = dm_kpts.shape[1]
    dm = numpy.array([numpy.sum(dm_kpts[0], axis=0), numpy.sum(dm_kpts[1], axis=0)], dtype=numpy.complex128)
    c0_up = _dot_ao_dm(ao, dm[0]) # (ngrid, nao) array
    c0_down = _dot_ao_dm(ao, dm[1])
    rho_up = numpy.einsum('pi,pi->p', ao.conj(), c0_up)
    rho_down = numpy.einsum('pi,pi->p', ao.conj(), c0_down)
    #rho_up = _contract_rho(ao, c0_up)
    #rho_down = _contract_rho(ao, c0_down)
    return numpy.real([rho_up, rho_down])
...
>>>>>>> 57d091ee8135b52b3a0f5ee4878d69b5368a5aa6
    ...
    Kwargs:
        ...
        weights : float
            used in `eval_rho` to check the electron number.
        conv_tol : float
            converge threshold.
        conv_tol_grad : float
